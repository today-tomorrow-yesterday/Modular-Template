...
2. **Naming:** `[EventDetailType("mt.{sourceModule}.{camelCaseEventName}")]`
...
[EventDetailType("mt.{sourceModule}.{camelCaseEventName}")]
...
|---|---|---|---|
| SampleOrders | Order | OrderPlaced | `mt.sampleOrders.orderPlaced` |
| SampleOrders | Order | OrderStatusChanged | `mt.sampleOrders.orderStatusChanged` |
| SampleSales | Product | ProductCreated | `mt.sampleSales.productCreated` |
| SampleSales | Product | ProductUpdated | `mt.sampleSales.productUpdated` |

## Checklist

- [ ] Event record has `Guid Public{EntityName}Id` — no int IDs
- [ ] `[EventDetailType("mt.{module}.{camelCase}")]` attribute present
