...
Docker Compose provides a way to run the full ModularTemplate stack locally, simulating the production ECS deployment with separate containers per module.
...
services:
  ...
  api:
    ...
    environment:
      ...
      - ConnectionStrings__Database=Host=postgres;Database=modulartemplate;Username=postgres;Password=postgres
