...
public class UpdatePackageConcessionsTests(SalesTestFactory factory) : SalesIntegrationTestBase(factory)
{
    ...
    public async Task Concession_WithAmount()
    {
        ...
        // Act
        var response = await Client.PutAsJsonAsync(Endpoint, new UpdatePackageConcessionsRequest(Amount: 3_000m));
        ...
        Assert.Equal(packageBeforeUpdate.GrossProfit - 3_000m, updatedPackage.GrossProfit); // Should reduce GP by concession amount
    ...
    public async Task Concession_AmountChanged()
    {
        ...
        var response1 = await Client.PutAsJsonAsync(Endpoint, new UpdatePackageConcessionsRequest(Amount: 3_000m));
        var response2 = await Client.PutAsJsonAsync(Endpoint, new UpdatePackageConcessionsRequest(Amount: 5_000m));
        ...
        Assert.Equal(packageBeforeUpdate.GrossProfit - 5_000m, updatedPackage.GrossProfit); // Should reduce GP by updated concession amount
    }
}
