...
public sealed class UpdateCustomerCacheHomeCenterCommandHandlerTests
{
    ...
    private readonly UpdateCustomerCacheHomeCenterCommandHandler _sut;

    public UpdateCustomerCacheHomeCenterCommandHandlerTests()
    {
        ...
        _sut = new UpdateCustomerCacheHomeCenterCommandHandler(_cacheWriteScope, _customerCacheWriter, _dateTimeProvider);
    }
    ...
    public async Task Returns_success_and_updates_home_center_number()
    {
        ...
        var command = new UpdateCustomerCacheHomeCenterCommand(RefPublicId: refPublicId, NewHomeCenterNumber: 200);

        var result = await _sut.Handle(command, CancellationToken.None);

        Assert.True(result.IsSuccess);
        await _customerCacheWriter.Received(1).UpdateHomeCenterNumberAsync(
