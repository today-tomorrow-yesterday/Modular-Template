...
// Flow: Inventory.OnLotHomePriceRevisedDomainEvent → publishes Inventory.OnLotHomePriceRevisedIntegrationEvent
internal sealed class OnLotHomePriceRevisedDomainEventHandler(
    IOnLotHomeRepository repository,
    IEventBus eventBus,
    IDateTimeProvider dateTimeProvider) : DomainEventHandler<OnLotHomePriceRevisedDomainEvent>
{
    public override async Task Handle(
        OnLotHomePriceRevisedDomainEvent domainEvent,
        CancellationToken cancellationToken = default)
    {
        ...
        await eventBus.PublishAsync(
            new OnLotHomePriceRevisedIntegrationEvent(
                Guid.CreateVersion7(),
