...
///
/// Common layers must follow: Rtl.Core.{Layer}
///   - e.g., Rtl.Core.Domain
...
public abstract class BaseTest
{
    ...
    // Matches: Rtl.Core.{Layer}
    private static readonly Regex CommonAssemblyPattern = new(
        @"^(Rtl\.Core)\.(Domain|Application|Infrastructure|Presentation)$",
        RegexOptions.Compiled);
    ...
    private static AssemblyInfo DiscoverAssemblies()
    {
        ...
        var referencedAssemblies = allAssemblies
            ...
            .Where(an => an.Name != null &&
                (an.Name.StartsWith("Rtl.Core.") || an.Name.StartsWith("Modules.")))
            .Distinct()
            .ToList();
